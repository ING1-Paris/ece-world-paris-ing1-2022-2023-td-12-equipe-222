...
int main() {
    ...
    tx3 = Ballon2->w;
    ty3 = Ballon2->h;
    posx3 = SCREEN_W / 2 - tx3 / 2;
    posy3 = SCREEN_H / 2 - ty3 / 2;
    depx3 = 16;
    depy3 = 10;

    tx4 = Ballon3->w;
    ty4 = Ballon3->h;
    posx4 = SCREEN_W / 2 - tx4 / 2;
    posy4 = SCREEN_H / 2 - ty4 / 2;
    depx4 = 43;
    depy4 = 40;

    tx5 = Ballon4->w;
    ty5 = Ballon4->h;
    posx5 = SCREEN_W / 2 - tx5 / 2;
    posy5 = SCREEN_H / 2 - ty5 / 2;
    depx5 = 19;
    depy5 = 20;
    ...
    tx7 = BallonF2->w;
    ty7 = BallonF2->h;
    posx7 = SCREEN_W / 2 - tx7 / 2;
    posy7 = SCREEN_H / 2 - ty7 / 2;
    depx7 = 12;
    depy7 = 9;
    ...
    // Dessiner le titre
    textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 4, makecol(255, 255, 255), "MENU PRINCIPAL");

    // Dessiner les options
    textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 2, makecol(255, 255, 255), "1. Jouer");
    textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 2 + 50, makecol(255, 255, 255), "2. Règles du jeu");
    ...
    switch (key) {
        case KEY_1:
            // Boucle interactive
            while (ballon_affiche1 == true || ballon_affiche2 == true || ballon_affiche3 == true ||
                   ballon_affiche4 == true || ballon_affiche5 == true || ballon_affiche6 == true ||
                   ballon_affiche7 == true || ballon_affiche8 == true) {
                ...
                if (mouse_b & 1 && mouse_x >= posx1 && mouse_x <= posx1 + Ballon1->w && mouse_y >= posy1 &&
                    mouse_y <= posy1 + Ballon1->h) {
                    // Effacement de l'image
                    ballon_affiche1 = false;
                }
                if (mouse_b & 1 && mouse_x >= posx3 && mouse_x <= posx3 + Ballon2->w && mouse_y >= posy3 &&
                    mouse_y <= posy3 + Ballon2->h) {
                    // Effacement de l'image
                    ballon_affiche2 = false;
                }
                if (mouse_b & 1 && mouse_x >= posx4 && mouse_x <= posx4 + Ballon3->w && mouse_y >= posy4 &&
                    mouse_y <= posy4 + Ballon3->h) {
                    // Effacement de l'image
                    ballon_affiche3 = false;
                }
                if (mouse_b & 1 && mouse_x >= posx5 && mouse_x <= posx5 + Ballon4->w && mouse_y >= posy5 &&
                    mouse_y <= posy5 + Ballon4->h) {
                    // Effacement de l'image
                    ballon_affiche4 = false;
                }
                textprintf_ex(page, font, 650, 20, makecol(0, 0, 0), -1, "Score : %d", score_ballon);
                if (ballon_affiche1 == false && ballon_affiche2 == false && ballon_affiche3 == false &&
                    ballon_affiche4 == false) {
                    ...
                    if (mouse_b & 1 && mouse_x >= posx6 && mouse_x <= posx6 + BallonF1->w && mouse_y >= posy6 &&
                        mouse_y <= posy6 + BallonF1->h) {
                        // Effacement de l'image
                        ballon_affiche5 = false;
                    }
                    if (mouse_b & 1 && mouse_x >= posx7 && mouse_x <= posx7 + BallonF2->w && mouse_y >= posy7 &&
                        mouse_y <= posy7 + BallonF2->h) {
                        // Effacement de l'image
                        ballon_affiche6 = false;
                    }
                    if (mouse_b & 1 && mouse_x >= posx8 && mouse_x <= posx8 + BallonF3->w && mouse_y >= posy8 &&
                        mouse_y <= posy8 + BallonF3->h) {
                        // Effacement de l'image
                        ballon_affiche7 = false;
                    }
                    if (mouse_b & 1 && mouse_x >= posx9 && mouse_x <= posx9 + BallonF4->w && mouse_y >= posy9 &&
                        mouse_y <= posy9 + BallonF4->h) {
                        // Effacement de l'image
        ...
        case KEY_2:
            clear_bitmap(page);
            textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 2 + 50, makecol(255, 255, 255), "Il existe plusieurs modes de jeu possibles pour le tir aux ballons forains : 2 couleurs de ballons sont placées dans le jeu. Chaque joueur dispose de 3 à 5 plombs pour crever les ballons de la couleur attribuée. Pour 1 seul jouer, 3 plombs pour 3 ballons : parviendrez vous à crever tous les ballons ? Tir de rapidité : 2 ballons de couleurs");
            blit(page, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
            rest(7000);
            clear_bitmap(page);
            // Dessiner le titre
            ...
            // Dessiner les options
            textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 2, makecol(255, 255, 255), "1. Jouer");
            textprintf_centre(page, font, SCREEN_W / 2, SCREEN_H / 2 + 50, makecol(255, 255, 255), "2. Quitter");
            blit(page, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
            while (!keypressed()) {
                rest(1);
            }
